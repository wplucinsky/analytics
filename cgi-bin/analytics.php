<?php
	...
	class Analytics {
		...
		function __construct(){
			...
			$this->extractGZ(); 			// extract all .gz data to .txt file
			$this->ipGrabber(); 			// extract all the ips to a .txt file
			$this->homepageConversion();	// gets the numbers of people who save an ad after getting to tool.php
			$this->scrollPercentages(); 	// gets the 15 most recent ads scroll percentages (ignores 0%)
			$this->dailyVisitors();			// gets the 30, 15, 10 and 1 day(s) num of visitors
			$this->device();				// gets the mobile vs. desktop device spread
			$this->algolia();				// uploads all the ad data to Algolia to be searched
		}

		public function dataRange($last){
		/* 
		...
		*/
			...
			// success prompt
				if ($this->displaySuccess) {echo 'extractGZ Success' . '<br>';}
		}

		public function ipGrabber(){
		/*
		...
		*/
			...
			// success prompt
				if ($this->displaySuccess) {echo 'ipGrabber Success' . '<br>';}
		}

		public function homepageConversion(){
		/*
		...
		*/
			...
			// create and check connections
				$conn1 = new mysqli($this->servername, $this->username, $this->password, $dbname1);
			...
			// success prompt
				if ($this->displaySuccess) {echo 'homepageConversion Success' . '<br>';}
		...
		*/
			...
			// create and check connections
				...
				$conn2 = new mysqli($this->servername, $this->username, $this->password, $dbname2);
			...
			// get 15 most recent ads
				$sql = "SELECT `id`, `fullname`, `title` FROM view ORDER BY `id` DESC LIMIT 60";
				$result = $conn1->query($sql);
				if ($result) {
					while ($row = $result->fetch_assoc()) {
					...
					if ($result) {
						while ($row = $result->fetch_assoc()) {
							if (array_key_exists('percentage', $row)) {
								if (in_array($row['percentage'], array_keys($percentages))){
			...
			// success prompt
				if ($this->displaySuccess) {echo 'scrollPercentages Success' . '<br>';}
		}
		...
		*/
			...
			// calculate day ranges
				$days30 = $this->dataRange(30);
				$days10 = $this->dataRange(10);
				$days5 = $this->dataRange(5);
				// $days1 = date('D M jS');
			...
			// success prompt
				if ($this->displaySuccess) {echo 'dailyVisitors Success' . '<br>';}
		}
		...
		*/
			...
			// success prompt
				if ($this->displaySuccess) {echo 'device Success' . '<br>'; }
		}
		...
		*/	
			...
			// create and check connections
				...
				// Algolia
					...
					$client = new \AlgoliaSearch\Client("FTG3PPP04U", "2da2f3479edbd3ee6e9826ec792bcdfe");
					$index = $client->initIndex('storageTable');

			// get information from `storage`
				$sql = "SELECT DISTINCT id, title, url, imgname, fullname, price, adtype FROM storage WHERE nsfw = 0 ORDER BY id ASC";
				$result = $conn1->query($sql);
					...
					while ($row = $result->fetch_assoc()) {
						...
						// upload to batch if views changed or file hasn't been uploaded already
							if ( ($record['views'] != @$viewsOrig[$key]) or !array_key_exists($key, $viewsOrig) ) {
								...
								$sql = "SELECT * FROM analytics WHERE (`url`= '{$record['url']}' AND `nickname`= '{$record['nickname']}')";
								$result2 = $conn1->query($sql);
								if( mysqli_num_rows($result2) < 1) {
									$sql = "INSERT INTO analytics (title, url, imgname, fullname, nickname, price, adtype, views, prevViews, dateAdded, dateUpdated, prevUpdated) VALUES ('{$record['title']}', '{$record['url']}', '{$row['imgname']}', '{$row['fullname']}', '{$record['nickname']}', '{$record['price']}', '{$record['adtype']}', '{$record['views']}', '0', '{$this->date->format('Y-m-d H:i:s')}', '{$this->date->format('Y-m-d H:i:s')}', '{$this->date->format('Y-m-d H:i:s')}')";
									$result2 = $conn1->query($sql);
								} else {
									$row = $result2->fetch_array(MYSQLI_ASSOC);
									// don't update previous views if aready updated today
									$today = new DateTime();
									if (current(explode(' ', $row['prevUpdated'])) == $today->format('Y-m-d')) {
										$sql = "UPDATE analytics SET `views` = '{$record['views']}', `dateUpdated` = '{$this->date->format('Y-m-d H:i:s')}' WHERE (`url`= '{$row['url']}' AND `nickname`= '{$record['nickname']}')";
									} else {
										$sql = "UPDATE analytics SET `views` = '{$record['views']}', `prevViews` = '{$row['views']}', `prevUpdated` = '{$this->date->format('Y-m-d H:i:s')}', `dateUpdated` = '{$this->date->format('Y-m-d H:i:s')}' WHERE (`url`= '{$row['url']}' AND `nickname`= '{$record['nickname']}')";
									}
									$result2 = $conn1->query($sql);
								}
								$i++;
			...
			// success prompt
				if ($this->displaySuccess) {echo 'algoliaUpload Success: ' . $i . '<br>';}
		}
	}
?>